...
void line_link::set_equal_edge_probability()
{
    ws = ransampl_alloc(edge_cnt);
    double *tmp_w = (double *)malloc(edge_cnt * sizeof(double));
    for (int k = 0; k != edge_cnt; k++)
    {
        tmp_w[k] = 1.0;
    }
    ransampl_set(ws, tmp_w);
}
...
void line_link::train_miniBatch_qa(real *_error_vec_u, real lr, real alpha, double (*func_rand_num)())
{
    int index, index2, u, vector_size;
    int pos = -1, neg = -1;
    real pos_value, neg_value;
    real margin = 1;
    ...
    if (edge_w[index] < 0.5)
        ...
        pos = edge_v[index];
    int setSize = 0, count = -1;
    for (unsigned int k = 0; k != graph[u].size(); k++)
    {
        if (graph[u][k].wei != edge_w[index])
            setSize += 1;
    }
    if (setSize == 0) return;
    ...
    //printf("%d\t%d\t%d\n", u, pos, neg);
    pos_value = node_u->vec.row(u) * node_v->vec.row(pos).transpose();
    //pos_value = pos_value / node_u->vec.row(u).norm() / node_v->vec.row(pos).norm();
    neg_value = node_u->vec.row(u) * node_v->vec.row(neg).transpose();
    //neg_value = neg_value / node_u->vec.row(u).norm() / node_v->vec.row(neg).norm();
    ...
    if (pos_value - neg_value < margin)
    {
        error_vec_u -= node_v->vec.row(neg) - node_v->vec.row(pos);
        node_v->vec.row(pos) += lr * (node_u->vec.row(u) - alpha * node_v->vec.row(pos));
        node_v->vec.row(neg) -= lr * (node_u->vec.row(u) + alpha * node_v->vec.row(neg));
    }
    node_u->vec.row(u) += lr * error_vec_u;
    return;
    //printf("%d\t%d\t%d\n", u, pos, neg);
