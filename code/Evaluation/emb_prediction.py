...
class Predicter_useFeatureEmb:
    ...
    def predict_types_for_rm_maximum(self, mention_id, candidate):
        ...
        return max_index, max_score, _emb_mention
    ...
    with open(os.path.join(indir + '/mention_feature_test.txt')) as f,\
        ...
        for line in f:
            ...
            if mention_id not in mentions_tested:
                ...
                label, score, _emb_mention = predicter.predict_types_for_rm_maximum(mention_id, all_candidates[mention_id])
                ...
                labels.append(label)
                scores.append(score)
                _emb_mentions.append(_emb_mention)
                cnt += 1

        scores_normalized = min_max_normalization(scores)
        # print scores_normalized
        for i in range(len(mentions)):
            #print scores_normalized[i]
            if scores_normalized[i] > _threshold:
                g.write(str(mentions[i])+'\t'+str(labels[i])+'\t'+ str(scores_normalized[i]) + '\t' +','.join([str(e) for e in _emb_mentions[i]]) +'\n')
                pos_cnt += 1
